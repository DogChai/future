...
class Circle {
    constructor(x, y) {
        ...
        this.colors = ['#FF1461', '#18FF92', '#5A87FF', '#FBF38C', '#f25da0', '#fb9627', '#d23be7', '#a3e048', '#4355db', '#ffb6bb', '#95d5ee', '#ffe691']
    ...
    moveFun(start, end, current) {
        let s = random(26, 35),
            ...
            ratio = end.x >= start.x ? (Math.max(current, ff) == current ? (Math.max(current, mm) ==
                current ? s : m) : f) : (Math.max(current, ff) == current ? f : (Math.max(current, mm) ==
                current ? m : s)),
            mp = {
                x: end.x - start.x,
                y: end.y - start.y
    ...
    move() {
        ...
        this.x = (this.opos.x > this.tpos.x) ? Math.max(this.x - movepos.x, this.tpos.x) : Math.min(this.x +
            movepos.x, this.tpos.x)
        this.y = this.opos.y > this.tpos.y ? Math.max(this.y - movepos.y, this.tpos.y) : Math.min(this.y +
            movepos.y, this.tpos.y)
...
window.clearRequestTimeout = window.cancelAnimationFrame || window.mozCancelRequestAnimationFrame || window.webkitCancelRequestAnimationFrame ||
